  ...
  if (value && typeof value === 'object') {
    return Object.entries(value).flatMap(([key, item]) => {
      ...
      if (Array.isArray(item)) {
        return [`${label}: ${item.map((entry) => typeof entry === 'object' ? JSON.stringify(entry) : String(entry)).join('; ')}`];
      }
      if (item && typeof item === 'object') {
        return asStringArray(item).map((entry) => `${label}: ${entry}`);
      }
      return item == null || String(item).trim() === '' ? [] : [`${label}: ${item}`];
    });
  }
  if (value == null) {
    return [];
...
function humanizeLabel(value) {
  const spaced = String(value || 'Action')
