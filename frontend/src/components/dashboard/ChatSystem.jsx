                  ...
                  <div ref={chatScrollRef} className="min-h-0 flex-1 overflow-y-auto px-4 py-5 md:px-7">
                    <div className="mx-auto flex w-full max-w-5xl flex-col gap-4">
                      ...
                      {selectedMessages.map((message) => {
                        const sender = normalizeMessageSender(message);
                        const isUser = sender === 'user';
                        const isAi = sender === 'ai';

                        return (
                          <div key={message.id} className={`group flex ${isUser ? 'justify-end' : 'justify-start'}`}>
                            <div className={`flex max-w-3xl items-end gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
                              {isAi && (
                              ...
                              {isUser && (
                                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-[#0f6b3d] text-xs font-semibold text-white shadow-sm">
                                  You
                                </div>
                              )}
                              <div className={`relative rounded-3xl px-4 py-4 text-sm leading-7 transition-all duration-200 ${isUser ? 'bg-[#0f6b3d] text-white shadow-[0_14px_32px_rgba(15,107,61,0.18)]' : isAi ? 'bg-[#f7f8f4] text-[#111827]' : 'border border-black/5 bg-white text-[#111827]'}`}>
                                <span className={`mb-2 block text-[10px] font-semibold uppercase tracking-[0.14em] ${isUser ? 'text-white/70' : 'text-[#7b817c]'}`}>
                                  {isUser ? 'Your prompt' : isAi ? 'Sentinal AI' : 'Snapshot context'}
                                </span>
                                <span className="block whitespace-pre-wrap">{message.text}</span>
                                {isAi && Array.isArray(message.toolsUsed) && message.toolsUsed.length > 0 && (
                                ...
                                <span className={`pointer-events-none absolute -bottom-6 text-[11px] uppercase tracking-[0.14em] opacity-0 transition-opacity duration-200 group-hover:opacity-100 ${isUser ? 'left-2 text-[#7b817c]' : 'right-2 text-[#7b817c]'}`}>
                                  {message.timestamp}
                                </span>
                              </div>
                            </div>
...
function normalizeMessageSender(message) {
  const raw = String(message?.sender || message?.role || message?.type || '').toLowerCase();
  if (raw === 'user' || raw === 'human' || raw === 'operator') {
    return 'user';
  }
  if (raw === 'ai' || raw === 'assistant' || raw === 'bot' || raw === 'sentinal') {
    return 'ai';
  }
  return 'system';
}
